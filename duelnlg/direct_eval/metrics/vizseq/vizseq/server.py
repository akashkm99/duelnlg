...
class TaskListHandler(VizSeqBaseRequestHandler):
    def get(self):
        enum_tasks_and_names_and_enum_models = VizSeqWebView.get_enum_tasks_and_names_and_enum_models(
            args.data_root
        )
...
class AboutHandler(VizSeqBaseRequestHandler):
    def get(self):
        html = env.get_template("about.html").render(version=__version__)
        self.write(html)
