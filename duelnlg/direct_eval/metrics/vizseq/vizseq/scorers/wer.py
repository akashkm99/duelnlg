...
def _get_sent_ins(
    ...
    return [
        float(get_wer(r, h).insertion) for r, h in zip(joined_references, hypothesis)
    ]
...
def _get_sent_del(
    ...
    joined_references = list(zip(*references))
    return [
        float(get_wer(r, h).deletion) for r, h in zip(joined_references, hypothesis)
    ]
...
def _get_sent_sub(
    ...
    joined_references = list(zip(*references))
    return [
        float(get_wer(r, h).substitution) for r, h in zip(joined_references, hypothesis)
    ]
