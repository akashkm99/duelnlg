...
def encode_and_serialize(
    input_file, output_file, vocab_file, do_lower_case, max_seq_length
):
...
def encode_and_serialize_from_list(
    references, candidates, output_file, tokenizer, max_seq_length
):
    """Encodes and serializes a set of references and candidates."""

    n_records = len(references)
    logging.info("Read {} examples.".format(n_records))
    ...
    with tf.python_io.TFRecordWriter(output_file) as writer:
        ...
        for i in range(n_records):
            ...
            if iterator_id % iterator_cycle == 0:
                logging.info("Writing example %d of %d", iterator_id, n_records)
            tf_example = serialize_example(
                references[i], candidates[i], tokenizer, max_seq_length, score=0
            )
            writer.write(tf_example)
    logging.info("Done writing {} tf examples.".format(n_records))
